...
extension EnvironmentValues {
	var navigateTo: NavigateToAction {
...
private struct ContentView: View {
	...
	@State var topLevelRoute: Route? = .start
	@State private var columnVisibility = CompatNavigationSplitViewVisibility.doubleColumn
	...
	@MainActor class FoldersRouteManager: ObservableObject {
		@Published var route: [Route] = []
	}
	@StateObject private var foldersTabRouteManager = FoldersRouteManager()
	...
	@ViewBuilder private func splitBody() -> some View {
		CompatNavigationSplitView(
					...
					if horizontalSizeClass != .compact {
						Section {
							if #available(iOS 16, *) {
								...
								NavigationLink(destination: StartOrSearchView(topLevelRoute: $topLevelRoute)) {
									Label("Start", systemImage: self.appState.syncState.systemImage)
								}
			...
			detail: {
				...
				case nil:
					CompatContentUnavailableView("Select a folder", systemImage: "folder")
						.onTapGesture {
	...
	var body: some View {
		...
		#if os(iOS)
			.onChange(of: QuickActionService.shared.action) { newAction in
			...
			if let folder = self.appState.client.folder(withID: folderID) {
				if folder.exists() {
					if let entry = try? folder.getFileInformation(path) {
						...
						CompatContentUnavailableView("File not found", systemImage: "document")
					}
				}
				else {
				...
				CompatContentUnavailableView("Folder not found", systemImage: "folder")
			}
