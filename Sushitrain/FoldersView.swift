...
struct FoldersSections: View {
			...
			ForEach(folders, id: \.self.folderID) { (folder: SushitrainFolder) in
				if !userSettings.hideHiddenFolders || folder.isHidden == false {
					if #available(iOS 16, *) {
						...
						NavigationLink(destination: RouteView(route: .folder(folderID: folder.folderID, prefix: nil))) {
							folderLabel(folder)
						}
						.contextMenu {
							folderContextMenu(folder)
	...
	@ViewBuilder private func folderLabel(_ folder: SushitrainFolder) -> some View {
		if folder.isPaused() {
	...
	@ViewBuilder private func folderContextMenu(_ folder: SushitrainFolder) -> some View {
