...
@MainActor class AppState: ObservableObject {
	...
	nonisolated let client: SushitrainClient
	...
	@Published var isLoggingToFile: Bool = false

	@Published private(set) var userSettings = AppUserSettings()
	@Published private(set) var localDeviceID: String = ""
	@Published private(set) var eventCounter: Int = 0
	@Published private(set) var foldersWithExtraFiles: [String] = []
	@Published private(set) var startupState: AppStartupState = .notStarted
	@Published private(set) var isMigratedToNewDatabase: Bool = false
	@Published private(set) var syncState: SyncState = SyncState(isDownloading: false, isUploading: false, connectedPeerCount: 0)
	@Published private(set) var launchedAt = Date.now
	@Published private(set) var maintenanceManager: MaintenanceManager!

	@Published fileprivate(set) var discoveredDevices: [String: [String]] = [:]
	@Published fileprivate(set) var resolvedListenAddresses = Set<String>()
	@Published fileprivate(set) var streamingProgress: StreamingProgress? = nil
	@Published fileprivate(set) var lastChanges: [SushitrainChange] = []
	@Published fileprivate(set) var currentNetworkPath: NWPath? = nil
